...
function withRuntimeMonitor(html: string, windowId: string) {
  const monitor = `<script>(function(){if(window.__praxisRuntimeMonitorInstalled)return;window.__praxisRuntimeMonitorInstalled=true;function post(payload){try{parent.postMessage(Object.assign({windowId:${JSON.stringify(windowId)}},payload),'*')}catch(_e){}}function send(error){post({type:'praxis-runtime-error',error:String(error||'Unknown runtime error')})}function reportSize(){try{var doc=document.documentElement;var body=document.body;var width=Math.max(doc?doc.scrollWidth:0,doc?doc.offsetWidth:0,body?body.scrollWidth:0,body?body.offsetWidth:0);var height=Math.max(doc?doc.scrollHeight:0,doc?doc.offsetHeight:0,body?body.scrollHeight:0,body?body.offsetHeight:0);post({type:'praxis-content-size',width:width,height:height})}catch(_e){}}window.addEventListener('error',function(event){send(event.message||event.error&&event.error.message||'Runtime error')});window.addEventListener('unhandledrejection',function(event){var reason=event.reason;send(reason&&reason.message?reason.message:String(reason||'Unhandled promise rejection'))});window.addEventListener('load',reportSize);window.addEventListener('resize',reportSize);if(window.ResizeObserver&&document.body){new ResizeObserver(reportSize).observe(document.body)}setTimeout(reportSize,50);setTimeout(reportSize,250);})();</script>`
